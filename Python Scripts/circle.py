"""
Circular flight example that connects to one crazyflie and flies it in a circular formation.

This program is designed to run with the Loco Positioning System setup in TWR mode
"""
import time
import csv
import datetime
...
def run_sequence(scf, sequence_to, sequence_land):
    cf = scf.cf

    cf.param.set_value('flightmode.posSet', '1')
    time.sleep(5.0) #Wait to initialize the loco system
    for position in sequence_to:
...
if __name__ == '__main__':
    ...
    with SyncCrazyflie(uri, cf=Crazyflie(rw_cache='./cache')) as scf:
        ...
        run_sequence(scf, sequence_to, sequence_land)
